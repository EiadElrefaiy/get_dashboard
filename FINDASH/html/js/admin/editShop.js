...
window.onload = function () {
    ...
    var shopId = localStorage.getItem("editShopPage");
    fetch("http://app.getcenter.info/api/v1/admins/get-shop-id", {
                ...
                .then((data) => {
                    console.log(data); 
                    document.getElementById("shopName").value = data["shops"]["name"];
                    document.getElementById("shopPhone").value = data["shops"]["phone"];
                    document.getElementById("shopAddress").value = data["shops"]["address"];
                    document.getElementById("shopLong").value = data["shops"]["longitude"];
                    document.getElementById("shopLat").value = data["shops"]["latitude"];
                    document.getElementById("shopLogo").value = data["shops"]["logo"];
                    document.getElementById("showShopLogo").src = data["shops"]["logo"];
                    document.getElementById("position").value = data["shops"]["position"];
                    ...
                    for (var i = 0; i < select.options.length; i++) {
                        option = select.options[i];
                             if (option.text == data["shops"]["state"]) {
                            option.setAttribute('selected', true);
                            stateChanged();
                            // For a single select, the job's done
                        }
                    }
                    var selectCenter = document.getElementById('centerSelect');
                    var optionCenter;
                    for (var i = 0; i < selectCenter.options.length; i++) {
                        optionCenter = selectCenter.options[i];
                             if (optionCenter.text == data["shops"]["center"]) {
                                optionCenter.setAttribute('selected', true);
                                centerChanged();
                            // For a single select, the job's done
                        }
                    }
                    ...
                    for(var n = 0; n <= data["images"].length - 1; n++){
                        $("#imageNode").append(' <div class="col-sm-3"> <div class="card iq-mb-3"> <img height="190" src="'+data["images"][n]["img"]+'" class="card-img-top" alt="#"> <div class="card-body"> <a onclick=deleteShopImage('+data["images"][n]["id"]+'); class="btn btn-danger" style="color: white;">حذف</a> </div> </div> </div>');
                    }
                });    
            }
        }
            ...
            function  deleteShopImage(id){
                fetch("http://app.getcenter.info/api/v1/admins/delete-shop-image", {
                    method: 'POST',
                    body: JSON.stringify({
                        id : id,
                        api_password: "tQnyBMCfK32bUx6pUnIh5IzR",
          ...
          function  editShop(){
            var shopId = localStorage.getItem("editShopPage");
                fetch("http://app.getcenter.info/api/v1/admins/update-shop", {
                    method: 'POST',
                    body: JSON.stringify({
                        id : shopId,
                        ...
                        latitude : document.getElementById("shopLong").value,
          ...
          function  editShopLogo(){
            var shopId = localStorage.getItem("editShopPage");
                fetch("http://app.getcenter.info/api/v1/admins/update-shop-logo", {
                    method: 'POST',
                    body: JSON.stringify({
                        id : shopId,
          ...
          function  addShopImage(){
            var shopId = localStorage.getItem("editShopPage");
                fetch("http://app.getcenter.info/api/v1/admins/add-shop-image", {
                    method: 'POST',
                    body: JSON.stringify({
                        id : shopId,
