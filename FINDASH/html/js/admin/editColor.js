window.onload = function(){
    ...
    fetch("http://app.getcenter.info/api/v1/admins/edit-color-id", {
        ...
        .then((data) => {
            ...
            console.log(data); 
            document.getElementById("description").value = data["color"][0]["description"];
            document.getElementById("url").value = data["color"][0]["picture"];
            document.getElementById("color").value = data["color"][0]["color"];
            document.getElementById("position").value = data["color"][0]["position"];
            document.getElementById('color_hexa').style.backgroundColor = '#' + data["color"][0]["color"];
            $("#modelNode").append('<div class="col-lg-12"> <div class="card iq-mb-3"> <div class="row no-gutters flex-row-reverse"> <div class="col-md-3"> <img width="300" height="400" src="'+data["color"][0]["picture"]+'"" alt="#"> </div> <div class="col-md-8"> <div class="card-body text-right"> <h4 class="card-title">'+data["color"][0]["category_name"]+'</h4> <p class="card-text">'+description+'</p> <p class="card-text"><small dir="rtl" class="text-muted" style="font-size: 20px;">'+data["color"][0]["price"]+' جنيه</small></p> </div>  </div> </div> </div> </div>');
           
            for(var n = 0; n <= data["sizes"].length - 1; n++){
                $("#sizesNode").append('<tr class="text-center"> <td><a class="btn btn-warning" style="color: white;" onclick="'+"localStorage.setItem('sizePage' , "+data["sizes"][n]["id"]+");window.location.assign('editSize.html');"+'">تعديل</a> &nbsp; <a style="color:white;" class="btn btn-danger"  onclick="'+"fetch('http://app.getcenter.info/api/v1/admins/delete-size', { method: 'POST', body: JSON.stringify({ id:" + data['sizes'][n]['id'] + ", api_password: 'tQnyBMCfK32bUx6pUnIh5IzR', }), headers: { 'Content-Type': 'application/json;charset=UTF-8' } }) .then((response) => { return response.json(); }) .then((data) => { console.log(data); window.location.reload(); });"+'">حذف</a></td> <td>'+data["sizes"][n]["quantity"]+'</td> <td>'+data["sizes"][n]["size"]+'</td> </tr>');
            }
       });        
    }
}
...
function updateColor(){
    var colorId = localStorage.getItem("colorPage");
    fetch("http://app.getcenter.info/api/v1/admins/update-color-id", {
        method: 'POST',
        body: JSON.stringify({
            id : colorId,
